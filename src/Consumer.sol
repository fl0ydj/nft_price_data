...
contract NFTBankConsumer is ChainlinkClient {
     ...
     */
    function fulfillPrice(bytes32 _requestId, uint256 _estimate)
        external
        recordChainlinkFulfillment(_requestId)
    {
        console2.log(_estimate);
    }

    function fulfillDateAndPrice(bytes32 _requestId, bytes32 _result)
        external
        recordChainlinkFulfillment(_requestId)
    {
        console2.log(getDateAndPrice(_result).date);
        console2.log(getDateAndPrice(_result).price);
    }
    ...
    function getDateAndFloorPrice(
        ...
        Chainlink.Request memory req = buildOperatorRequest(
            _specId,
            this.fulfillDateAndPrice.selector
        );
    ...
    function getDateAndPrice(bytes32 _data)
        private
        pure
        returns (DateAndPrice memory)
    {
        DateAndPrice memory dateAndPrice = DateAndPrice(
            uint128(bytes16(_data)),
            uint128(bytes16(_data << 128))
        );
        return dateAndPrice;
    }
